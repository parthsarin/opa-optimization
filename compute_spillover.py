...
rawdog = pd.read_csv('final_results_1000.csv')
rawdog.columns = [c.lower().replace(' ', '_') for c in rawdog.columns]

n = len(rawdog)
sim_ids = set(rawdog.simulation)
...
funding = 2_000_000
for spillover_amt in range(5, 101):
    print(f'progress: {spillover_amt} / 100')
    spillover_amt = spillover_amt / 100

    df = get_df(spillover_amt)
    nh, ph = thanos(funding, df)

    helped_overall = pd.concat([
        # df[df.eligible_market * df.viable == 1],
        df[df.viable_eligible == 1],
        ph
    ])
    helped_overall['spilled_over'] = helped_overall['price'] != helped_overall['adj_price']
    spillover = sum(
        helped_overall['spilled_over'] * (helped_overall['price'] - helped_overall['adj_price'])
    ) / num_sims
    print(f'{spillover = }')

    if spillover >= funding:
        break


print(f'Spillover (Breakeven): {spillover_amt:.0%}')