...
def perc_low_income(people_helped, full_df):
    ...
    # num_low_income_sfh = sum(
    #     full_df.eligible_market 
    #     * (1 - full_df.eligible_sfh)
    #     * full_df.viable 
    #     * (full_df.income_category == 'Low Income')
    # ) / num_sims
    ...
    return (num_low_income_afford + num_low_income_helped) / total_low_income # could add num_low_income_sfh
...
def perc_poc(people_helped, full_df):
    ...
    return (num_poc_afford + num_poc_helped) / total_poc # could add num_poc_sfh


# sfh = (df.eligible_market * (1 - df.eligible_sfh) * df.viable).sum() / num_sims
no_funding = df.groupby('simulation').sum().viable_eligible
p_ev = (no_funding + nh).mean() / num_per_sim # could add sfh

p_li_realistic = perc_low_income(ph, df)
p_li_realistic_equitable = perc_low_income(phe, df)

p_poc_realistic = perc_poc(ph, df)
p_poc_realistic_equitable = perc_poc(phe, df)

print(f'Eligibility & Viability: {p_ev:.2%}')
print(f'Income Equity (Realistic): {p_li_realistic:.2%}')
print(f'Income Equity (Realistic Equitable): {p_li_realistic_equitable:.2%}')
print(f'Racial Equity (Realistic): {p_poc_realistic:.2%}')
print(f'Racial Equity (Realistic Equitable): {p_poc_realistic_equitable:.2%}')